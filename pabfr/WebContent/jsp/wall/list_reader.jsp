...
<script>
	function loadNewPostForm(){
		this.$("#collapse_new_post").load('<%=request.getContextPath()%>/event/addReader');//collapse_new_post
	}
</script>
				...
				<s:iterator value="logList" status="ClientStatus" var="name">
		          <s:if test="typeobj != 'Gare'">
	                ...
	                <li>
		                ...
		                <s:else>
		                <s:if test="(#session.UserSessionKey.name == tolist.substring(29,tolist.indexOf(' - '))) || (creator == #session.UserSessionKey.name)">
					...
					<div class=""><s:text name="wall_page.no_news"/></div>
		                	...
		                	<!-- coment.substring(29,coment.indexOf('</a>')) 		                
		                	-->
